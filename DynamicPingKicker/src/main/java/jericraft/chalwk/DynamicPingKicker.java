...
public final class DynamicPingKicker extends JavaPlugin implements Listener {
    ...
    private int getPingLimit() {
        int count = this.getServer().getOnlinePlayers().size();

        for (Map<?, ?> opt : config.getMapList("limits")) {
            List<Integer> arr = new ArrayList<>((Collection<? extends Integer>) opt.values());
